...
DOES_ENTRY_EXIST=`cat $COMPOSER_HOME/config.json | jq --arg type $WERCKER_ADD_PRIVATE_COMPOSER_REPOSITORY_TYPE --arg url $WERCKER_ADD_PRIVATE_COMPOSER_REPOSITORY_URL '.repositories | map(contains({"type": $type, "url": $url})) | any'`
if [ "$DOES_ENTRY_EXIST" == "true" ]; then
    echo "Private repository entry already present... ignoring."
else
    # add repository entry
    cat $COMPOSER_HOME/config.json | jq --arg type $WERCKER_ADD_PRIVATE_COMPOSER_REPOSITORY_TYPE --arg url $WERCKER_ADD_PRIVATE_COMPOSER_REPOSITORY_URL  '.repositories |= .+ [{"type": $type, "url": $url}]' > result.json
    mv -f result.json $COMPOSER_HOME/config.json
fi
